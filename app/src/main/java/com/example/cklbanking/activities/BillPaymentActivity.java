...
public class BillPaymentActivity extends AppCompatActivity {
    ...
    private TextInputEditText editCustomerCode;
    ...
    private void setupBillTypeSpinner() {
        ...
        spinnerBillType.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                selectedBillType = BILL_TYPES[position];
            }

            @Override
            public void onNothingSelected(AdapterView<?> parent) {
                selectedBillType = BILL_TYPES[0];
            }
        });
        selectedBillType = BILL_TYPES[0];
    }
    ...
    private void processPayment() {
        ...
        if (TextUtils.isEmpty(customerCode)) {
            editCustomerCode.setError("Vui lòng nhập mã khách hàng");
            return;
        }
    ...
    private void performPayment(String customerCode, double amount) {
        ...
        String utilityType = selectedBillType.equals("Tiền điện") ? "electricity" : "water";
        String provider = selectedBillType.equals("Tiền điện") ? "EVN" : "SAVACO";
        ...
        utilityPayment.setCustomerCode(customerCode);
        utilityPayment.setProvider(provider);
        utilityPayment.setDescription("Thanh toán " + selectedBillType + " - Mã KH: " + customerCode);

        // Create transaction
        Transaction transaction = new Transaction();
        transaction.setFromAccountId(selectedAccount.getAccountId());
        transaction.setToAccountId(provider); // Provider as recipient
